...
namespace Veterinaria
{
    public partial class ReporteVenta : Form
    {
        ...
        {
            MySqlCommand cm = new MySqlCommand("Select * from clientes WHERE folio = '" + textBoxVentas.Text.Trim() + "'",
        ...
        void llenarGrid()
        {
            MySqlCommand cm = new MySqlCommand("Select * from venta", Conexion.ObtenerConexion());
            MySqlDataAdapter da = new MySqlDataAdapter(cm);
            DataTable dt = new DataTable();
            da.Fill(dt);
            dataGridView1.DataSource = dt;
